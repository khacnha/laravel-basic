        ...
        <div class="box-header">
            ...
            <div class="box-tools">
                ...
                <div class="input-group" style="width: 150px;">
                    <input type="text" value="{{\Request::get('search')}}" class="form-control input-sm" name="search" placeholder="{{ __('message.user.search_keyword') }}">
                ...
                <a href="{{ url('/admin/users/create') }}" class="btn btn-success btn-sm" title="{{ __('message.new_add') }}">
                    ...
                    <tr>
                        ...
                        <td>
                            @foreach ($item->roles as $index=>$role)
                                <span class="badge label-{{ $role->color }}">{{ $role->name }}</span>
                            @endforeach
                        </td>
                        <td>{{ $item->active==Config("settings.active")?__('message.yes'):__('message.no') }}</td>
                        <td>
                            <a href="{{ url('/admin/users/' . $item->id) }}" title="View User"><button class="btn btn-info btn-xs"><i class="fa fa-eye" aria-hidden="true"></i> {{ __('message.view') }}</button></a>
                            <a href="{{ url('/admin/users/' . $item->id . '/edit') }}" title="Edit User"><button class="btn btn-primary btn-xs"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> {{ __('message.edit') }}</button></a>
                            {!! Form::open([
                                'method' => 'DELETE',
